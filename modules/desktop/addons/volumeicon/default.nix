...
let cfg = config.antob.desktop.addons.volumeicon;
in {
  options.antob.desktop.addons.volumeicon = with types; {
    enable =
      mkEnableOption "Whether or not to install and configure volumeicon.";
  };

  config = mkIf cfg.enable {
    antob.home.extraOptions = {
      home.packages = [ pkgs.volumeicon pkgs.at-spi2-atk ];
