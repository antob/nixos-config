...
pkgs.writeShellScriptBin "hypr-bitwarden-resize" ''
  #!/bin/bash
    ...
    case $1 in
      windowtitle*)
        ...
        if [[ "$window_title" == *"(Bitwarden Password Manager) - Bitwarden"* ]]; then
          ...
          hyprctl --batch "dispatch setprop address:0x$window_id no_anim on ; dispatch setfloating address:0x$window_id ; dispatch resizewindowpixel exact 500 800,address:0x$window_id"
        fi
        ;;
    esac
  }
