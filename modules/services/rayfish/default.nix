...
let
  cfg = config.antob.services.rayfish;
...
in
{
  options.antob.services.rayfish = {
    ...
    operator = lib.mkOption {
      ...
      default = config.antob.user.name;
      example = "alice";
      description = ''
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      {
        ...
        message = "antob.services.rayfish.apply: set either `spec` or `specFile`, not both.";
      }
    ];
    ...
    antob.home.extraOptions.programs.zsh.initContent = ''
      eval "$(${lib.getExe pkgs.rayfish} completions zsh)"
    '';
    ...
    systemd.services.rayfish-settings = lib.mkIf wantSettingsUnit {
      ...
      script = ''
        ...
        ${lib.optionalString (
          cfg.operator != null
        ) "${ray} set-operator ${lib.escapeShellArg cfg.operator}"}
    ...
    antob.persistence.directories = [ "/etc/rayfish" ];
  };
}
