...
let
  ...
  other-hosts = lib.filterAttrs
    (key: host:
      key != name && (host.config.antob.user.name or null) != null)
...
in
{
  ...
  config = mkIf cfg.enable {
    services.openssh = {
      ...
      settings.PermitRootLogin = if format == "install-iso" then "yes" else "no";
    ...
    antob.user.extraOptions.openssh.authorizedKeys.keys =
      cfg.authorizedKeys;
  };
}
